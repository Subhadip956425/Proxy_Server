...
app.get("/coins/markets", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "CoinGecko proxy error", detail: error.message });
  }
});
...
app.get("/coins/:id", async (req, res) => {
  try {
    const response = await axios.get(
      `${COINGECKO_API}/coins/${req.params.id}`,
      {
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
          Accept: "application/json",
...
app.get("/coins/:id/market_chart", async (req, res) => {
  try {
    const response = await axios.get(
      `${COINGECKO_API}/coins/${req.params.id}/market_chart`,
      {
        params: req.query,
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
    ...
    res
      .status(500)
      .json({ error: "Market chart proxy error", detail: error.message });
  }
});
...
app.get("/search/trending", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Trending search proxy error", detail: error.message });
  }
});
